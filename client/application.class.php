...
public function login($username, $password, $mysqli) {
    ...
    if ($stmt = $mysqli->prepare("SELECT id, email, password, secret_key 
        FROM users
       WHERE username = ?
        LIMIT 1")) {
        ...
        $stmt->bind_result($user_id, $email, $db_password, $secret_key);
        $stmt->fetch();
        // hash the password with the unique salt.
        $password = hash('sha512', $password . $secret_key);
        if ($stmt->num_rows == 1) {
...
{
    private $string = '';
    private $english = array("a", "e", "s", "S","ş","Ş", "A", "o", "O", "t", "i", "ph", "H", "W", "M", "D", "V", "x");
    private $leet = array("4", "3", "5", "5","$","$","4", "0", "0", "7", "1", "f", "|-|", "\\/\\/", "|\\/|", "|)", "\\/", "><");
